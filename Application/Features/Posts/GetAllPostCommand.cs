  ...
  public class GetAllPostCommandHandler : IRequestHandler<GetAllPostCommand, Response<List<GetAllPostCommandResponse>>>
  {
    ...
    public async Task<Response<List<GetAllPostCommandResponse>>> Handle(GetAllPostCommand request, CancellationToken cancellationToken)
    {
      try
      {
        ...
        var postResponses = posts.Select(post => new GetAllPostCommandResponse(post.Id, post.User.Username, post.User.ProfilePicture, post.MediaUrl, post.Caption)).ToList();
...
public class GetAllPostCommandResponse
{
  ...
  public GetAllPostCommandResponse(Guid id, string username, string userProfilePicture, string mediaUrl, string? caption)
  {
    Id = id;
    Username = username;
